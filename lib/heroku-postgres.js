...
module.exports = function (localDatabaseURL) {
    ...
    db.query = function (query, data, callback, client) {
        if (client) runQuery(client, query, data, callback);
        else db.connect(function (err, client) {
            if (err) return callback(err);
            runQuery(client, query, data, callback);
        })
    };

    var runQuery = function (client, query, data, callback) {
        ...
        client.query(
            ...
            function (err, data) {
                ...
                queryAction.finish();
                // the client is passed to the callback so it can be re-used for nested queries
                callback(err, data, client);
            }
        );
    };
